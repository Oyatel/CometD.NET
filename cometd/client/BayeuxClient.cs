...
namespace Cometd.Client
{
	...
	public class BayeuxClient: AbstractClientSession, IBayeux
	{
		...
		protected void processMessage(IMutableMessage message)
		{
			...
			if (Channel_Fields.META_HANDSHAKE.Equals(channelName))
				processHandshake(message);
			else if (Channel_Fields.META_CONNECT.Equals(channelName))
				processConnect(message);
			else if (Channel_Fields.META_DISCONNECT.Equals(channelName))
		...
		protected void processHandshake(IMutableMessage handshake)
		{
			if (handshake.Successful)
			{
				...
				if (newTransport == null)
				{
					...
					// Signal the failure
					String error = "405:c" + transportRegistry.AllowedTransports + ",s" + serverTransports.ToString() + ":could not negotiate transport with server";
		...
		{
            System.Timers.Timer timer = new System.Timers.Timer(); // @@ax: WARNING hold rede på flere timere på en gang?
        ...
        {
            // @@ax: WARNING, should return a copy
            get
            {
                return options;
            }
		...
		protected void enqueueSend(IMutableMessage message)
		{
			...
			BayeuxClientState bayeuxClientState = this.bayeuxClientState;
			bool handshaking = isHandshaking(bayeuxClientState);
			if (batching || handshaking)
			{
				messageQueue.Enqueue(message);
				//Console.WriteLine("Enqueued message {0} (batching: {1})", message, batching);
			}
			else
			{
        ...
        private void updateBayeuxClientState(NewBayeuxClientStateDelegate newBayeuxClientState)
		{
			BayeuxClientState newState = null;
			BayeuxClientState oldState = bayeuxClientState;
				
			newState = newBayeuxClientState(oldState);
			if (newState == null)
        ...
        private delegate BayeuxClientState NewBayeuxClientStateDelegate(BayeuxClientState oldState);
